...
RSpec.context 'When logged out' do
  ...
  describe 'access to one user' do
    it 'is successful' do
      get "/users/#{[*@some_users, *@some_admins].sample.id}"
  ...
  describe 'user creation' do
    ...
    it 'only creates regular users' do
      post '/users', params: { user: FactoryBot.attributes_for(:admin) }
  ...
  describe 'user update' do
    ...
    it 'fails with PATCH' do
      patch "/users/#{@new_user.id}", params: { user: { given_name: 'Testing Alice' } }
      expect(response).to have_http_status(:forbidden)
      expect(User.find(@new_user.id).given_name).to_not eq('Testing Alice')
    end

    it 'fails with PUT' do
      put "/users/#{@new_user.id}", params: { user: { family_name: 'Testing Smith' } }
      expect(response).to have_http_status(:forbidden)
    ...
    it 'fails to update the password even if old_password is provided correctly' do
      put "/users/#{@new_user.id}", params: { user: { old_password: 'testpassword', password: 'newpassword!' } }
      expect(response).to have_http_status(:forbidden)
    end
  end
  ...
  describe 'user login' do
    ...
    it 'is successful with the right credentials' do
      post '/login', params: { email: @new_user.email, password: 'testpassword' }
      expect(response).to have_http_status(:ok)
    end

    it 'fails with the wrong password' do
      post '/login', params: { email: @new_user.email, password: 'wrongpassword' }
      expect(response).to have_http_status(:forbidden)
    end

    it 'fails with the wrong credentials' do
      post '/login', params: { email: 'somerandomemail@example.com', password: 'wrongpassword' }
      expect(response).to have_http_status(:forbidden)
    end
  end
end
...
RSpec.context 'When logged in' do
  ...
  context 'as a user' do
    before(:each) do
      post '/login', params: { email: @user.email, password: 'testpassword'}
      @token = json['token']
    ...
    describe 'user creation' do
      ...
      it 'fails' do
        post '/users', params: { user: @new_user }, headers: { **@auth_headers }
        expect(response).to have_http_status(:forbidden)
      end
    end
    ...
    describe 'user update' do
      it 'succeeds with PATCH' do
        patch "/users/#{@user.id}", params: { user: { given_name: 'Testing Alice' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:ok)
        expect(json['given_name']).to eq('Testing Alice')
      end
  
      it 'succeeds with PUT' do
        put "/users/#{@user.id}", params: { user: { family_name: 'Testing Smith' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:ok)
        expect(json['family_name']).to eq('Testing Smith')
      end
  
      it 'updates the password when the old_password is correct' do
        put "/users/#{@user.id}", params: { user: { old_password: 'testpassword', password: 'newpassword!' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:ok)
      end

      it 'does not update the password when the old_password is wrong' do
        put "/users/#{@user.id}", params: { user: { old_password: 'wrongtestpassword', password: 'newpassword!' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:forbidden)
      end

      it 'fails to update own role' do
        put "/users/#{@user.id}", params: { user: { role: 'ADMIN' } }, headers: { **@auth_headers }
    ...
    describe 'other user update' do
      it 'fails with PATCH' do
        patch "/users/#{@other_user.id}", params: { user: { given_name: 'Testing Alice' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:forbidden)
        expect(User.find(@other_user.id).given_name).to_not eq('Testing Alice')
      end
  
      it 'fails with PUT' do
        put "/users/#{@other_user.id}", params: { user: { family_name: 'Testing Smith' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:forbidden)
      ...
      it 'does not update the password when the old_password is correct' do
        put "/users/#{@other_user.id}", params: { user: { old_password: 'othertestpassword', password: 'newpassword!' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:forbidden)
      end

      it 'does not update the password when the old_password is wrong' do
        put "/users/#{@other_user.id}", params: { user: { old_password: 'wrongtestpassword', password: 'newpassword!' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:forbidden)
      end

      it 'fails to update role' do
        put "/users/#{@other_user.id}", params: { user: { role: 'ADMIN' } }, headers: { **@auth_headers }
  ...
  context 'as an admin' do
    before(:each) do
      post '/login', params: { email: @admin.email, password: 'admintestpassword'}
      @token = json['token']
      @auth_headers = { Authorization: "Bearer #{@token}" }
    end
    ...
    describe 'user update' do
      it 'succeeds with PATCH' do
        patch "/users/#{@admin.id}", params: { user: { given_name: 'Testing Alice' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:ok)
        expect(json['given_name']).to eq('Testing Alice')
      end
  
      it 'succeeds with PUT' do
        put "/users/#{@admin.id}", params: { user: { family_name: 'Testing Smith' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:ok)
        expect(json['family_name']).to eq('Testing Smith')
      end
  
      it 'updates the password when the old_password is correct' do
        put "/users/#{@admin.id}", params: { user: { old_password: 'admintestpassword', password: 'newpassword!' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:ok)
      end

      it 'does not update the password when the old_password is wrong' do
        put "/users/#{@admin.id}", params: { user: { old_password: 'wrongtestpassword', password: 'newpassword!' } }, headers: { **@auth_headers }
        expect(response).to have_http_status(:forbidden)
      end

      it 'can update own role' do
        put "/users/#{@admin.id}", params: { user: { role: 'USER' } }, headers: { **@auth_headers }
